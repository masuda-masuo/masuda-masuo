...
*   **コアバリュー**: 実装担当の小型モデル（`DeepSeek Flash`等）のコード変更に対し、別セッションの独立したレビューモデル（`Claude Fable`等）による敵対レビューを自動ループさせ、テスト全緑になるまで自律的に再実装を往復させる「**実装 ➔ レビュー ➔ 差し戻し**」の自動化チェーン（`chain` コマンド）を構築します。
